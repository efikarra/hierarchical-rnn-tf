...
"graph", "model", "input_file_placeholder", "output_file_placeholder", "iterator"))):
    pass
...
def is_hierarchical(model_architecture):
    if model_architecture == "h-rnn-ffn" or model_architecture == "h-rnn-rnn" or model_architecture == "h-rnn-cnn" or \
        ...
        return True
    elif model_architecture == "ffn" or model_architecture == "rnn" or model_architecture == "cnn":
        return False
    else:
        raise ValueError("Unknown model architecture %s." % model_architecture)
...
def rnn_network(inputs, dtype, rnn_type,
                unit_type, num_units, num_layers, hid_to_out_dropout, sequence_length, forget_bias,
                activations, mode):
    """Builds a RNN network."""
    if rnn_type == 'uni':
        rnn_outputs, last_hidden_sate = rnn(inputs, dtype, unit_type, num_units, num_layers, hid_to_out_dropout,
                                            sequence_length, forget_bias, activations, mode)
    elif rnn_type == 'bi':
...
def rnn(inputs, dtype, unit_type, num_units, num_layers, hid_to_out_dropout, sequence_length, forget_bias,
    ...
    # The RNN calculations stop when the true seq. length is reached for each sequence.
    # All outputs (hidden states) past the true seq length are set to 0.
    # The hidden state of the true last timestep is returned as last_hidden_state
...
def ffn(inputs, layers, units_list, bias, hid_to_out_dropouts, activations, mode):
    ...
    for l in range(layers):
        ...
        layer_output = tf.layers.Dense(units_list[l], activation=get_activation_func(activations[l]), use_bias=bias)(
            layer_input)
...
def pool_rnn_output(pooling_method, rnn_outputs, rnn_last_state, sequence_length, attention_size=32):
    ...
    if pooling_method == 'last':
        if isinstance(rnn_last_state, tf.contrib.rnn.LSTMStateTuple):
            return rnn_last_state.h, attn_alphas
        else:
...
def get_initializer(init_op, seed=None, init_weight=None):
    """Create an initializer. init_weight is only used for uniform."""
    if init_op == "uniform":
        assert init_weight
...
def run_batch_evaluation_and_prediction(model, session):
    ...
    loss = 0.0
    accuracy = 0.0
    concat_predictions = {"probabilities": [], "labels": []}
...
def create_or_load_model(model, session, name, model_dir, input_emb_weights=None):
    # if there is a checkpoint in the model_dir, load the model using this ckpt.
